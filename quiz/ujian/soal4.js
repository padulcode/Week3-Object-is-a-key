...
function graduates (students) {
  ...
  // menambahkan object dalam object kelas
  for (const i of students) {
    kelas[i.class] = [{name : null, score : null}];
  }

  // push nilai tertinggi lainnya dari tiap class
  for (const key in kelas) {      
    for (const i of students) {
      if(i.class == key && i.score > 75){
        kelas[key].push({ name: i.name , score: i.score });
        sumLulus ++;
      }
    }
  }
  //shift nilai null karena pembuatan object class
  for (const key in kelas) {
    for (let i = 0; i < kelas[key].length; i++) {
      if(kelas[key].name == null){
        kelas[key].shift();
        break;
      }
    }
  ...
  //jika tidak ada student yang lulus / student tidak ada
  if (students.length == 0 || sumLulus == 0) {
    return []
  }
