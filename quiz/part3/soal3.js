...
function countProfit(shoppers) {
    ...
    for (const key in items) {
      ...
      for (const i of shoppers) {
        if(i.product == items[key].product){
          if(items[key].leftOver - i.amount >= 0){
            items[key].shoppers.push(i.name); // push nama-nama pembeli
            items[key].leftOver -= i.amount; // menghitung jumlah stock
            for (let j = 0; j < listBarang.length; j++) {
              if(listBarang[j][0] == i.product){
                items[key].totalProfit += listBarang[j][1] * i.amount; // menghitung jumlah profit
              }
            }
          }
        }
